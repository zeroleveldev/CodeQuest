...
class Enemy:
    def __init__(self, name: str, max_health: int, base_damage: int = 5 ,loot_table: list[dict] = None):
        self.name = name
            ...
            if random.random() < drop.get("chance", 1.0):
                if drop["type"] == "gold":
                    ...
                    player.inventory.add_item(
                        drop["item"],
                        drop.get("data", {"attack":0, "defense":0, "max_hp_bonus":0, "lore":"A mysterious find"}),
                        drop.get("amount", 1)
                    )
