...
def main():
    ...
    while True:
        ...
        if last_direction:
            opp = {"north":"south", "south":"north", "east":"west", "west":"east"}.get(last_direction, last_direction)
            print(f"    (came from the {opp})")
        ...
        if command == "quit":
            ...
            if not rest:
                ...
                print("Slots: main_hand, off_hand, head, shoulders, chest, gloves, pants, boots, necklace, ring")
                continue
            ...
            if player.unequip(slot):
                # Extra message for two-hand weapons
                if slot == "main_hand" and player.equipment.get("main_hand") is None:
                    if player.equipment.get("off_hand"):
                        print("(Off-hand is now usable again.)")
